...
var interactions = [
  {
    role: "crewMate",
    message:
      "Hi ! Welcome to CrewMate. Seems like you have built a great Crew! Kickoff the crew with your tasks now !",
  },
  {
    role: "user",
    message: "",
...
const AgentTaskLinesList = ({ agentTaskData }) => (
  <>
    {agentTaskData.map((agent, index) => (
      <AgentTaskLines
        key={index}
        message={`${index + 1}. ${agent.agent_name} - ${agent.agent_task}`}
      />
    ))}
  </>
);
...
const CrewInteractionContainer = ({ crewId, agents }) => {
  ...
  useEffect(() => {
    interactions[1].message = (
      <AgentTaskLinesList agentTaskData={agentTaskData} />
    );
    interactions[2].message =
      "Enjoy the animation while the intelligent system is spinning up some impactful work for you !";
    setModalShow(false);
  }, [agentTaskData]);
      ...
      <footer className="d-flex justify-content-center">
        <div className="d-flex col-4 justify-content-between mt-4">
          <Button
            ...
            text={CREW_INTERACTION_PAGE.buttons.assignTask.label}
            onClick={() => setModalShow(true)}
          />
          <Button
            className={"primary-reverse block"}
            text={CREW_INTERACTION_PAGE.buttons.kickoff.label}
            isDisabled={interactions.length < 1}
          />
        </div>
      </footer>
      <FormModal
        show={modalShow}
        title={CREW_INTERACTION_PAGE.modal.header.assignTasks}
        agents={agents}
        crewId={crewId}
        onHide={() => setModalShow(false)}
        setAgentTaskData={setAgentTaskData}
        setHasFinalSummary={setHasFinalSummary}
