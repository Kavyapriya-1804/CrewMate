...
import { submitCrew, submitTask } from "../../service/CrewService";
...
const AgentTasks = ({
  ...
  const handleSubmit = (e) => {
    ...
    submitAgentTasksForm(formattedData, setHasFinalSummary, setIsResultLoading);
  };

  const submitAgentTasksForm = async (
    formattedData,
    setHasFinalSummary,
    setIsResultLoading
  ) => {
    var dataFetched = false;
    const crewData = await submitTask(formattedData)
      .then(() => {
        setIsResultLoading(true);
      ...
      if(dataFetched){
        // const report = await kickoffCrew(crewId).then(() => {
  ...
  return (
    <form
      className="agent-task-inputs-wrapper d-flex flex-column align-items-center"
    ...
    >
      {agents &&
        agents.map((agent, index) => (
          <div className="d-flex flex-column col-12 mb-4" key={index}>
            ...
            <input
              type="text"
              className="agent-task mt-3 ps-3"
              name="agent_task"
              id={agent.agent_id}
      ...
      <Button
        type={"submit"}
        className={"primary-reverse block my-2 text-uppercase"}
    ...
    >
      ...
      >
        <Modal.Title
          ...
          className="col-11 ms-5 text-center"
        >
          {title}
        </Modal.Title>
